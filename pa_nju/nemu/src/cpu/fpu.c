...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
    /*
    sign:结果的符号
    exp：中间结果的阶数（含偏置函数），因为是中间结果，故可能为负
    sig_grs:26位，中间结果运算尾数
    */
	// normalization
	bool overflow = false; // true if the result is INFINITY or 0 during normalize

	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)//需要进行右规且未发生阶码上溢
	{
		// normalize toward right
		 /* (需要右规 且 未发生阶码上溢) 或（扩充条件 exp < 0 且 尾数没有被右规至舍入后为0，即，sig_grs > 0x4） */ 
		 /* 右规一次 或称 逐级下溢一次*/
			   ...
			   )//0x04==0100
		{
			...
			uint32_t sticky=sig_grs&0x1;//取最后一位grs作为sticky
		
		
			if(sticky==1)
		    {
		        sig_grs>>=1;
		        sig_grs=sig_grs|0x1;//把sticky粘上去
		        //如果用+1可能出问题进位，这种置0置1的操作最好还是用位运算
		        exp++;
		...
		if (exp >= 0xff)//exp加过了头（ >= 0xFF了），阶码上溢
		{
	...
	else if (((sig_grs >> (23 + 3)) == 0) && exp > 0)//需要进行左归
	{
	...
	if (!overflow)
	{
		...
		uint32_t tmp=sig_grs&0x07;//取后三位，保护位，舍入位，粘位
		sig_grs>>=3;
	     if(tmp==4)//0010 0舍1入
		{
		    if((sig_grs&1)==1)
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	/* TODO: shift = ? */
	//讨论非规格化数的情形=================== 阶码真值变为-126
	if(fa.exponent==0&&fb.exponent==0){
	    shift=(fb.exponent+1)-(fa.exponent+1);
	}
	else if(fa.exponent==0){
	      shift=fb.exponent-(fa.exponent+1);
	}
	else if(fb.exponent==0){
	     shift=fb.exponent+1-fa.exponent;
	}
	else{
